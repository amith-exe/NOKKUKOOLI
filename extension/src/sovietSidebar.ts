...
import { HardwareStatus } from "./serialManager";

export interface SovietSidebarState {
  isLocked: boolean;
...
export class SovietSidebarViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = "nokkukooli.sovietPanel";
  private _view?: vscode.WebviewView;

  constructor(
    private readonly _extensionUri: vscode.Uri,
    private readonly _getState: () => SovietSidebarState,
    private readonly _onDispatchAgent: (prompt: string) => Promise<boolean>,
    private readonly _onSelectPort: () => void,
  ) {}
  ...
  public updateStatus(
    ...
    hardwareStatus: HardwareStatus,
    isCopilotAvailable: boolean = true,
  ) {
    if (this._view) {
      this._view.webview.postMessage({
        type: "UPDATE_TELEMETRY",

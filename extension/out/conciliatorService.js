...
exports.ConciliatorService = void 0;
exports.negotiateWithConciliator = negotiateWithConciliator;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class ConciliatorService {
    outputChannel;
    constructor() {
        this.outputChannel =
     ...
     */
    async negotiate(userPlea, audioBase64, mimeType) {
        this.loadLocalEnvironment();
        const config = vscode.workspace.getConfiguration("nokkukooli");
        const apiKey = process.env.GEMINI_API_KEY || config.get("geminiApiKey") || "";
        this.outputChannel.appendLine(`[CONCILIATOR] Incoming plea: "${userPlea}" (Audio attached: ${!!audioBase64})`);
        ...
        if (!apiKey || apiKey === "PLACEHOLDER_KEY" || apiKey.trim() === "") {
            this.outputChannel.appendLine("[CONCILIATOR] No Gemini API key provided. Returning fallback satirical response.");
            if (userPlea === "INITIAL_VIOLATION_ROAST") {
                return {
                    reply: "സഖാവേ, അനുമതി ഇല്ലാതെ കീബോർഡിൽ കൈയെത്തിച്ചല്ലോ. ഇതിന് ശാസ്‌ത്യമാർന്നു; ആദ്യഘട്ടത്തിൽ ഒരു കോയിന് മൂന്ന് മിനിറ്റ് (180 സെക്കൻഡ്) ലീസ് എടുത്താൽ മതിയാകും. കൂടുതൽ സമയം വേണമെങ്കിൽ ചർച്ചയിൽ ഉയർത്താം.",
                    permitSeconds: 180,
                    leaseSeconds: 180,
                    coinCost: 1,
                };
            ...
            if (userPlea === "LEASE_EXPIRED_ROAST") {
                return {
                    reply: "സഖാവേ, ലീസ് കാലാവധി കഴിഞ്ഞു; ഇപ്പോഴും നിങ്ങളുടെ ബാക്കിയിരിക്കുന്ന കോയിൻകൾ എണ്ണത്തിൽ നിലനിൽക്കുന്നു. അതുകൊണ്ടു തന്നെയാ, വീണ്ടും ചർച്ച ചെയ്ത് കുറഞ്ഞതും നീതി പുലർത്തിയതുമായ ലീസിന് പോകേണ്ടത്; പുതുക്കാൻ ഒരു കോയിന് മൂന്ന് മിനിറ്റ് എന്ന നിരക്ക് തന്നെ ലഭിക്കും.",
                    permitSeconds: 180,
                    leaseSeconds: 180,
                    coinCost: 1,
                };
            ...
            if (lower.includes("sorry") ||
                ...
                return {
                    reply: "സഖാവേ, അടിയന്തര സാഹചര്യമാണെന്ന് മനസ്സിലായി. ഈ അവസരത്തിൽ ചെറിയ ഇളവ് കാണിക്കാം: രണ്ടു കോയിന് ആറ് മിനിറ്റ് (360 സെക്കൻഡ്) ലീസ്. പ്രോഡക്ഷൻ ഭീഷണി ഓടുന്നതിനാൽ ഈ തുക തന്നെ ശാന്തമായി സ്വീകരിക്കാം.",
                    permitSeconds: 360,
                    leaseSeconds: 360,
                    coinCost: 2,
                };
            ...
            if (lower.includes("more") ||
                ...
                return {
                    reply: "സഖാവേ, കൂടുതൽ സമയം വേണമെന്ന് മനസ്സിലായി. രണ്ട് കോയിന് ആറ് മിനിറ്റ് (360s) നും അധികമായി, മൂന്നുരണ്ട് നീണ്ട ലീസ് വേണമെങ്കിൽ മൂന്നാം കോയിൻ കൂടി ചേർക്കാം; അതാണ് ന്യായമായ ചർച്ച.",
                    permitSeconds: 540,
                    leaseSeconds: 540,
                    coinCost: 3,
                };
            ...
            return {
                reply: "സഖാവേ, നിലവിലെ യൂണിയൻ നിരക്ക് ഒരു കോയിന് മൂന്ന് മിനിറ്റാണ്. അതനുസരിച്ച് രണ്ട് കോയിന് ആറ് മിനിറ്റും മൂന്ന് കോയിന് ഒൻപത് മിനിറ്റും ലീസ് ലഭിക്കും; തുടരാൻ ഡീൽ ഉറപ്പിക്കുക.",
                permitSeconds: 180,
                leaseSeconds: 180,
                coinCost: 1,
            };
        ...
        try {
            const systemPrompt = `You are the office conciliator, a rigid, mildly theatrical Malayalam clerk overseeing typing permits in a bureaucratic office.\n` +
            ...
            for (const modelName of candidateModels) {
                try {
                    ...
                    if (res.ok) {
                        ...
                        this.outputChannel.appendLine(`[CONCILIATOR] Model ${modelName} returned status ${res.status}: ${lastError}`);
                    }
                }
                catch (e) {
                    lastError = e.message;
            ...
            const fallbackSeconds = requestedMins > 0 ? requestedMins * 60 : fallbackCoins * 180;
            return {
...
exports.ConciliatorService = ConciliatorService;
/**
 * Standalone helper matching the blueprint interface
 */
async function negotiateWithConciliator(userPlea) {
    const service = new ConciliatorService();
    return await service.negotiate(userPlea);
}
//# sourceMappingURL=conciliatorService.js.map